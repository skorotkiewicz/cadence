...
pub fn load_staged(dir: &Path) -> Result<StagedFiles> {
    ...
    let staged: StagedFiles = serde_json::from_str(&content)
        .with_context(|| "Failed to parse staged.json")?;
...
pub fn save_staged(dir: &Path, staged: &StagedFiles) -> Result<()> {
    ...
    let content = serde_json::to_string_pretty(staged)
        .with_context(|| "Failed to serialize staged")?;
...
pub fn load_db(dir: &Path) -> Result<Database> {
    ...
    let db: Database = serde_json::from_str(&content)
        .with_context(|| "Failed to parse db.json")?;
...
pub fn save_db(dir: &Path, db: &Database) -> Result<()> {
    ...
    let content = serde_json::to_string_pretty(db)
        .with_context(|| "Failed to serialize db")?;
